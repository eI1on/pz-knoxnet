...
function PacmanGame:onDeactivate()
end
...
function PacmanGame:preview(x, y, width, height, terminal, gamesModule)
    ...
    local pacmanTexture = getTexture("media/ui/Games/pacman/pacman_left.png")
...
function PacmanGame:render(gamesModule)
    ...
    terminal:renderTitle("PACMAN - LEVEL: " ..
        self.gameState.level .. " | SCORE: " .. self.gameState.score .. " | LIVES: " .. self.gameState.pacman.lives)
    ...
    for y = 1, Constants.PACMAN_CONST.GRID_HEIGHT do
        for x = 1, Constants.PACMAN_CONST.GRID_WIDTH do
            ...
            if self.gameState.board[y][x] == Constants.PACMAN_CONST.ENTITIES.WALL then
                terminal:drawRect(
                    cellX, cellY,
                    self.gameState.cellSize, self.gameState.cellSize,
    ...
    if self.gameState.pacman.direction.x == Constants.PACMAN_CONST.DIRECTIONS.LEFT.x and
        self.gameState.pacman.direction.y == Constants.PACMAN_CONST.DIRECTIONS.LEFT.y then
        pacmanTexturePath = Constants.PACMAN_CONST.TEXTURES.PACMAN_LEFT
    elseif self.gameState.pacman.direction.x == Constants.PACMAN_CONST.DIRECTIONS.UP.x and
        self.gameState.pacman.direction.y == Constants.PACMAN_CONST.DIRECTIONS.UP.y then
        pacmanTexturePath = Constants.PACMAN_CONST.TEXTURES.PACMAN_UP
    elseif self.gameState.pacman.direction.x == Constants.PACMAN_CONST.DIRECTIONS.DOWN.x and
        self.gameState.pacman.direction.y == Constants.PACMAN_CONST.DIRECTIONS.DOWN.y then
    ...
    for i = 1, #self.gameState.ghosts do
        ...
        if ghost.mode == "frightened" then
            ...
            if self.gameState.ghostFrightenedEndTime > 0 and
                (self.gameState.ghostFrightenedEndTime - getTimeInMillis() < 2000) and
                (self.gameState.animationFrame == 1) then
                r, g, b = 1, 1, 1
            end
    ...
    if self.gameState.gameOver then
        ...
        terminal:drawText(
            ...
            1, self.gameState.gameWon and 0.3 or 1, self.gameState.gameWon and 1 or 0.3, 0.3,
...
function PacmanGame:isValidPacmanPosition(x, y)
    if x < 1 or x > Constants.PACMAN_CONST.GRID_WIDTH or
        y < 1 or y > Constants.PACMAN_CONST.GRID_HEIGHT then
        if y == 14 then
            if x < 1 then
...
function PacmanGame:getOppositeDirection(direction)
    if direction.x == Constants.PACMAN_CONST.DIRECTIONS.UP.x and
        direction.y == Constants.PACMAN_CONST.DIRECTIONS.UP.y then
        return Constants.PACMAN_CONST.DIRECTIONS.DOWN
    elseif direction.x == Constants.PACMAN_CONST.DIRECTIONS.DOWN.x and
        direction.y == Constants.PACMAN_CONST.DIRECTIONS.DOWN.y then
        return Constants.PACMAN_CONST.DIRECTIONS.UP
    elseif direction.x == Constants.PACMAN_CONST.DIRECTIONS.LEFT.x and
        direction.y == Constants.PACMAN_CONST.DIRECTIONS.LEFT.y then
        return Constants.PACMAN_CONST.DIRECTIONS.RIGHT
    elseif direction.x == Constants.PACMAN_CONST.DIRECTIONS.RIGHT.x and
        direction.y == Constants.PACMAN_CONST.DIRECTIONS.RIGHT.y then
